...
from .expressions.constraint import Constraint
from .expressions.expression import Expression
from .expressions.objective import ObjectiveType
...
class Model:
    ...
    def _create_dual_variables(self, primal, dual):
        ...
        for i in range(1, len(primal.constraints)+1):
            dual.create_variable("y"+i.__str__())
    ...
    def _create_dual_objective(self, primal, dual):
        ...
        dual.objective = ob.Objective(Expression.from_vectors(dual.variables, factors=bounds), ob.ObjectiveType.MIN)
    ...
    def _create_dual_constraints(self, primal, dual):
        ...
        for i in range(0, len(primal.variables)):
            factors = []
            for constraint in primal.constraints:
                factors.append(constraint.expression.factors(primal)[i])
            dual.add_constraint(Constraint(
                                            expression=Expression.from_vectors(dual.variables, factors),
                                            bound=primal.objective.expression.factors(primal)[i]))
